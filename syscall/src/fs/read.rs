use core::sync::atomic::Ordering;
use los_mm::user as mm_user;

use los_vfs::dispatch::*;
use los_vfs::error::VfsError;
// TEAM_420: Direct linux_raw_sys imports, no shims
// TEAM_421: Return SyscallResult directly - NO ERRNO CASTS
// TEAM_422: Arc<dyn Any> downcasting for type-erased FdType handles
use crate::{SyscallResult, write_to_user_buf};
use linux_raw_sys::errno::{EBADF, EFAULT, EINVAL, EIO};
use los_fs_tty::pty::PtyPair;
use los_sched::fd_table::FdType;

/// TEAM_217: struct iovec for writev/readv
#[repr(C)]
#[derive(Clone, Copy)]
struct UserIoVec {
    base: usize,
    len: usize,
}

/// TEAM_217: sys_readv - Vectored read.
/// TEAM_421: Returns SyscallResult directly - NO ERRNO CASTS.
pub fn sys_readv(fd: usize, iov_ptr: usize, count: usize) -> SyscallResult {
    if count == 0 {
        return Ok(0);
    }
    if count > 1024 {
        return Err(EINVAL);
    }

    let task = los_sched::current_task();
    let ttbr0 = task.ttbr0.load(Ordering::Acquire);

    // Validate iovec array
    let iov_size = count * core::mem::size_of::<UserIoVec>();
    if mm_user::validate_user_buffer(ttbr0, iov_ptr, iov_size, false).is_err() {
        return Err(EFAULT);
    }

    let mut total_read: i64 = 0;

    for i in 0..count {
        let entry_addr = iov_ptr + i * core::mem::size_of::<UserIoVec>();
        let iov = unsafe {
            let kptr = match mm_user::user_va_to_kernel_ptr(ttbr0, entry_addr) {
                Some(p) => p,
                None => return Err(EFAULT),
            };
            *(kptr as *const UserIoVec)
        };

        if iov.len == 0 {
            continue;
        }

        // TEAM_421: Call sys_read which returns SyscallResult directly
        match sys_read(fd, iov.base, iov.len) {
            Ok(n) => {
                total_read += n;
                if n < iov.len as i64 {
                    // Short read, stop here
                    break;
                }
            }
            Err(e) => {
                if total_read == 0 {
                    return Err(e);
                } else {
                    return Ok(total_read);
                }
            }
        }
    }

    Ok(total_read)
}

/// TEAM_081: sys_read - Read from a file descriptor.
/// TEAM_178: Refactored to dispatch by fd type, added InitramfsFile support.
/// TEAM_421: Returns SyscallResult directly - NO ERRNO CASTS.
pub fn sys_read(fd: usize, buf: usize, len: usize) -> SyscallResult {
    if len == 0 {
        return Ok(0);
    }

    let task = los_sched::current_task();

    // TEAM_178: Look up fd type and dispatch accordingly
    let fd_table = task.fd_table.lock();
    let entry = match fd_table.get(fd) {
        Some(e) => e.clone(),
        None => return Err(EBADF),
    };
    drop(fd_table);

    let ttbr0 = task.ttbr0.load(Ordering::Acquire);

    match entry.fd_type {
        FdType::Stdin => {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[SYS_READ] fd={} type=Stdin len={}", fd, len);
            read_stdin(buf, len, ttbr0)
        }
        FdType::VfsFile(ref file) => {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[SYS_READ] fd={} type=VfsFile len={}", fd, len);
            if mm_user::validate_user_buffer(ttbr0, buf, len, true).is_err() {
                return Err(EFAULT);
            }
            let mut kbuf = alloc::vec![0u8; len];
            match vfs_read(file, &mut kbuf) {
                Ok(n) => {
                    let dest = match mm_user::user_va_to_kernel_ptr(ttbr0, buf) {
                        Some(p) => p,
                        None => return Err(EFAULT),
                    };
                    unsafe {
                        core::ptr::copy_nonoverlapping(kbuf.as_ptr(), dest, n);
                    }
                    Ok(n as i64)
                }
                Err(VfsError::BadFd) => Err(EBADF),
                Err(_) => Err(EIO),
            }
        }
        FdType::PipeRead(ref pipe) => {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[SYS_READ] fd={} type=PipeRead len={}", fd, len);
            if mm_user::validate_user_buffer(ttbr0, buf, len, true).is_err() {
                return Err(EFAULT);
            }
            let mut kbuf = alloc::vec![0u8; len];
            // TEAM_421: Pipe now returns Result<usize, u32> directly
            let n = pipe.read(&mut kbuf)?;
            let dest = match mm_user::user_va_to_kernel_ptr(ttbr0, buf) {
                Some(p) => p,
                None => return Err(EFAULT),
            };
            unsafe {
                core::ptr::copy_nonoverlapping(kbuf.as_ptr(), dest, n);
            }
            Ok(n as i64)
        }
        FdType::PtyMaster(ref pair_any) => {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[SYS_READ] fd={} type=PtyMaster len={}", fd, len);
            // TEAM_422: Downcast Arc<dyn Any> to PtyPair
            let pair = match pair_any.downcast_ref::<PtyPair>() {
                Some(p) => p,
                None => return Err(EBADF),
            };
            let mut bytes_read = 0;
            loop {
                let mut buffer = pair.master_read_buffer.lock();
                if let Some(byte) = buffer.pop_front() {
                    if !write_to_user_buf(ttbr0, buf, bytes_read, byte) {
                        return Err(EFAULT);
                    }
                    bytes_read += 1;
                    if bytes_read >= len {
                        return Ok(bytes_read as i64);
                    }
                    continue;
                }

                if bytes_read > 0 {
                    return Ok(bytes_read as i64);
                }
                drop(buffer);

                // Wait for output
                unsafe {
                    los_hal::interrupts::enable();
                }
                let _ = los_hal::interrupts::disable();
                los_sched::yield_now();
            }
        }
        FdType::PtySlave(ref pair_any) => {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[SYS_READ] fd={} type=PtySlave len={}", fd, len);
            // TEAM_422: Downcast Arc<dyn Any> to PtyPair
            let pair = match pair_any.downcast_ref::<PtyPair>() {
                Some(p) => p,
                None => return Err(EBADF),
            };
            read_from_tty(&pair.tty, buf, len, ttbr0, false)
        }
        _ => {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[SYS_READ] fd={} type=Other(EBADF?)", fd);
            Err(EBADF)
        }
    }
}

/// TEAM_178: Read from stdin (keyboard/console input).
/// TEAM_247: Refactored to use TTY line discipline.
/// TEAM_421: Returns SyscallResult directly.
fn read_stdin(buf: usize, len: usize, ttbr0: usize) -> SyscallResult {
    use los_fs_tty::CONSOLE_TTY;
    read_from_tty(&CONSOLE_TTY, buf, len, ttbr0, true)
}

/// TEAM_421: TTY read implementation returning SyscallResult directly.
fn read_from_tty(
    tty_mutex: &los_utils::Mutex<los_fs_tty::TtyState>,
    buf: usize,
    len: usize,
    ttbr0: usize,
    is_console: bool,
) -> SyscallResult {
    let max_read = len.min(4096);
    if mm_user::validate_user_buffer(ttbr0, buf, max_read, true).is_err() {
        return Err(EFAULT);
    }

    loop {
        // 1. Feed hardware input into TTY (only for console)
        if is_console {
            poll_to_tty();
        }

        // 2. Try to satisfy read from TTY input buffer
        let mut tty = tty_mutex.lock();
        if !tty.input_buffer.is_empty() {
            #[cfg(feature = "verbose-syscalls")]
            los_hal::println!("[TTY] Input buffer has {} bytes", tty.input_buffer.len());

            let mut bytes_read = 0;
            while bytes_read < max_read {
                if let Some(byte) = tty.input_buffer.pop_front() {
                    if !write_to_user_buf(ttbr0, buf, bytes_read, byte) {
                        return Err(EFAULT);
                    }
                    bytes_read += 1;

                    // In canonical mode, read() returns at most one line per call
                    if (tty.termios.c_lflag & los_fs_tty::ICANON) != 0 && byte == b'\n' {
                        return Ok(bytes_read as i64);
                    }
                } else {
                    break;
                }
            }
            return Ok(bytes_read as i64);
        }
        drop(tty);

        unsafe {
            los_hal::interrupts::enable();
        }
        let _ = los_hal::interrupts::disable();

        los_sched::yield_now();
    }
}

fn poll_to_tty() {
    use los_fs_tty::CONSOLE_TTY;

    // TEAM_422: Input polling is handled at the kernel level.
    // The input module (VirtIO keyboard/mouse) is in the main kernel.
    // The syscall crate only reads from the HAL console which is always available.

    while let Some(byte) = los_hal::console::read_byte() {
        log::trace!("[TTY] poll_to_tty got byte: {:#x}", byte);
        CONSOLE_TTY.lock().process_input(byte);
    }
}
