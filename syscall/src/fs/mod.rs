// use crate::memory::user as mm_user;

mod dir;
mod fd;
mod link;
mod mount;
mod open;
mod read;
mod stat;
mod statx;
mod write;

// Re-export all syscall functions
pub use dir::{sys_getcwd, sys_getdents, sys_mkdirat, sys_renameat, sys_unlinkat};
pub use fd::{sys_dup, sys_dup2, sys_dup3, sys_fcntl, sys_ioctl, sys_isatty, sys_pipe2};
pub use fd::{sys_lseek, sys_chdir, sys_fchdir, sys_ftruncate, sys_truncate, sys_pread64, sys_pwrite64};
pub use fd::{sys_chmod, sys_fchmod, sys_chown, sys_fchown, sys_fchmodat, sys_fchownat};
pub use link::{sys_linkat, sys_readlinkat, sys_symlinkat, sys_utimensat};
pub use mount::{sys_mount, sys_umount};
pub use open::{sys_close, sys_faccessat, sys_openat};
pub use read::{sys_read, sys_readv};
pub use stat::{sys_fstat, sys_fstatat};
pub use statx::sys_statx;
pub use write::{sys_write, sys_writev};
