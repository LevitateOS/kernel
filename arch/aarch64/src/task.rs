...
/// TEAM_422: Exception return function for thread creation.
/// This function restores the SyscallFrame from SP and performs eret.
/// Used by los_sched::thread::create_thread to set up the initial return point.
///
/// # Safety
/// - SP must point to a valid SyscallFrame
/// - Called only from thread entry point after context switch
#[unsafe(no_mangle)]
pub unsafe extern "C" fn exception_return() -> ! {
    // The SyscallFrame is at SP. Restore registers and eret.
    // This is a simplified version - the full implementation would be in assembly.
    unsafe {
        core::arch::asm!(
            // Load the SyscallFrame from SP
            // Frame layout: regs[31], sp, pc, pstate, ttbr0
            // Restore x0-x30, sp, pc, pstate
            "ldp x0, x1, [sp, #0]",
            "ldp x2, x3, [sp, #16]",
            "ldp x4, x5, [sp, #32]",
            "ldp x6, x7, [sp, #48]",
            ...
            "ldr x17, [sp, #248]",
            "msr sp_el0, x17",
            // Load pc -> elr_el1 (offset 256)
            "ldr x17, [sp, #256]",
            "msr elr_el1, x17",
            // Load pstate -> spsr_el1 (offset 264)
            "ldr x17, [sp, #264]",
            "msr spsr_el1, x17",
            // Skip ttbr0 (already set)
            // Adjust SP past the frame (31*8 + 4*8 = 280 bytes)
            "add sp, sp, #280",
            // Return to userspace
            "eret",
            options(noreturn)
