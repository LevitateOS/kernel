...
pub fn sys_spawn_args(path_ptr: usize, path_len: usize, argv_ptr: usize, argc: usize) -> i64 {
    ...
    if argc > 0
        && crate::task::user_mm::validate_user_buffer(task.ttbr0, argv_ptr, argv_size, false)
            .is_err()
    {
        return errno::EFAULT;
    }
    ...
    for i in 0..argc {
        ...
        let entry = unsafe {
            let kernel_ptr = crate::task::user_mm::user_va_to_kernel_ptr(task.ttbr0, entry_ptr);
            match kernel_ptr {
        ...
        if crate::task::user_mm::validate_user_buffer(task.ttbr0, entry.ptr, arg_len, false)
            .is_err()
        {
            return errno::EFAULT;
        }
...
pub fn sys_waitpid(pid: i32, status_ptr: usize) -> i64 {
    ...
    if let Some(exit_code) = crate::task::process_table::try_wait(pid) {
        ...
        if status_ptr != 0 {
            // Validate and write
            if crate::task::user_mm::validate_user_buffer(current.ttbr0, status_ptr, 4, true)
                .is_ok()
            {
                if let Some(ptr) =
                    crate::task::user_mm::user_va_to_kernel_ptr(current.ttbr0, status_ptr)
                {
                    unsafe {
