...
// TEAM_208: user_mm moved to los_mm::user
...
pub extern "C" fn task_exit() -> ! {
    ...
    if clear_tid != 0 {
        ...
        // TODO(TEAM_422): futex_wake creates circular dep with los_syscall
        // For now, use the futex hook if set, otherwise skip
        let wake_fn = FUTEX_WAKE_HOOK.load(Ordering::Acquire);
        if !wake_fn.is_null() {
            unsafe {
                let wake: fn(usize, i32) -> i32 = core::mem::transmute(wake_fn);
                wake(clear_tid, 1);
            }
        }
    }
...
/// TEAM_422: Hook for futex_wake to avoid circular dependency with los_syscall.
/// Set by los_syscall during init. Function signature: fn(usize, i32) -> i32
pub static FUTEX_WAKE_HOOK: AtomicPtr<()> = AtomicPtr::new(core::ptr::null_mut());

/// TEAM_422: Register the futex_wake function from los_syscall.
pub fn set_futex_wake_hook(hook: fn(usize, i32) -> i32) {
    FUTEX_WAKE_HOOK.store(hook as *mut (), Ordering::Release);
}
...
pub fn current_task() -> Arc<TaskControlBlock> {
    ...
    unsafe {
        ...
        if !ptr.is_null() {
            ...
            let arc: Arc<TaskControlBlock> = Arc::from_raw(ptr);
...
pub struct TaskControlBlock {
    ...
    pub vmas: IrqSafeLock<los_mm::vma::VmaList>,
